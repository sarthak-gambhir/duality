...
import { CommandPalette, type Command } from '../src/components/command_palette/CommandPalette';

function makeCommands(onSave = vi.fn(), onCopy = vi.fn()): Command[] {
  return [
    { id: 'save', label: 'Save', group: 'File', keywords: ['write'], onSelect: onSave },
...
describe('CommandPalette', () => {
  it('renders a searchable dialog only when open', () => {
    const { rerender } = render(
      <CommandPalette isOpen={false} onClose={() => {}} commands={makeCommands()} />,
    );
    expect(screen.queryByRole('dialog')).not.toBeInTheDocument();

    rerender(<CommandPalette isOpen onClose={() => {}} commands={makeCommands()} />);
  ...
  it('filters by label and keywords as the user types', async () => {
    ...
    render(<CommandPalette isOpen onClose={() => {}} commands={makeCommands()} />);
  ...
  it('runs the active command on Enter and closes', async () => {
    ...
    const user = userEvent.setup();
    render(
      <CommandPalette isOpen onClose={onClose} commands={makeCommands(onSave)} />,
    );
