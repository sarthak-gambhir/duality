    ...
    <span ref={rootRef} className="du_menu_root">
      ...
      {isOpen && (
        <div
          ...
          id={menuId}
          aria-label={ariaLabel}
          className={cx('du_menu', `du_menu_${placement.replace('-', '_')}`, className)}
...
export function MenuItem({ onSelect, disabled, className, onClick, ...rest }: MenuItemProps) {
...
export function MenuSeparator({ className, ...rest }: ComponentPropsWithoutRef<'div'>) {
  return <div role="separator" className={cx('du_menu_separator', className)} {...rest} />;
}
